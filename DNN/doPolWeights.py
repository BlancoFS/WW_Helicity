...
def compute_weights(GenPart_eta, GenPart_pt, GenPart_mass, GenPart_phi, pos_mother, status, particles):
  ...
  weight_f0_Wm = (3.0/4.0) * f0_m * (1 - cos_Wm_theta_star*cos_Wm_theta_star)
  weight_fL_Wm = (3.0/8.0) * fL_m * (1 + cos_Wm_theta_star)*(1 + cos_Wm_theta_star)
  weight_fR_Wm = (3.0/8.0) * fR_m * (1 - cos_Wm_theta_star)*(1 - cos_Wm_theta_star)
  weight_fT_Wm = (3.0/8.0) * fL_m * (1 + cos_Wm_theta_star)*(1 + cos_Wm_theta_star) + (3.0/8.0) * fR_m * (1 - cos_Wm_theta_star)*(1 - cos_Wm_theta_star)
  weight_total_Wm = weight_f0_Wm + weight_fL_Wm + weight_fR_Wm

  # W plus
  
  weight_f0_Wp = (3.0/4.0) * f0_p * (1 - cos_Wp_theta_star*cos_Wp_theta_star)
  weight_fL_Wp = (3.0/8.0) * fL_p * (1 - cos_Wp_theta_star)*(1 - cos_Wp_theta_star)
  weight_fR_Wp = (3.0/8.0) * fR_p * (1 + cos_Wp_theta_star)*(1 + cos_Wp_theta_star)
  weight_fT_Wp = (3.0/8.0) * fL_p * (1 - cos_Wp_theta_star)*(1 - cos_Wp_theta_star) + (3.0/8.0) * fR_p * (1 + cos_Wp_theta_star)*(1 + cos_Wp_theta_star)
...
if __name__ == "__main__":
    ...
    print("\n")
    print("======================================")
    print("Running Double Polarization reweighter")
    print("======================================")
    ...
    for event in range(0, len(df)):
        ...
        w_LL.append(weights_result[0])
        w_TL.append(weights_result[1])
        w_LT.append(weights_result[2])
        w_TT.append(weights_result[3])
        ...
        recWp.SetCoordinates(W_p.Pt(), W_p.Eta(), W_p.Phi(), W_p.E())
        recWm.SetCoordinates(W_m.Pt(), W_m.Eta(), W_m.Phi(), W_m.E())
        ...
        leppWRF = ROOT.Math.XYZVector()
        lepmWRF = ROOT.Math.XYZVector()
        leppWRF = ROOT.Math.VectorUtil.boost(reclp, recwpRF)
        lepmWRF = ROOT.Math.VectorUtil.boost(reclm, recwmRF)
        
        theta_Wp_star = ROOT.Math.VectorUtil.Angle(leppWRF, recWp)
        theta_Wm_star = ROOT.Math.VectorUtil.Angle(lepmWRF, recWm)
        
        cos_Wp_theta_star = np.cos(theta_Wp_star)
        cos_Wm_theta_star = np.cos(theta_Wm_star)

        cos_theta_p.append(cos_Wp_theta_star)
        cos_theta_m.append(cos_Wm_theta_star)
        ...
        pos_theta_Wp_star = ROOT.Math.VectorUtil.Angle(leppWRF, recWp_pos)
        pos_theta_Wm_star = ROOT.Math.VectorUtil.Angle(lepmWRF, recWm_pos)
        ...
        neg_theta_Wp_star = ROOT.Math.VectorUtil.Angle(leppWRF, recWp_neg)
        neg_theta_Wm_star = ROOT.Math.VectorUtil.Angle(lepmWRF, recWm_neg)
    ...
    df.loc[:, "weight_LL"] = df.weight_LL * df.XSWeight * df.METFilter_MC
    df.loc[:, "weight_TL"] = df.weight_TL * df.XSWeight * df.METFilter_MC
    df.loc[:, "weight_LT"] = df.weight_LT * df.XSWeight * df.METFilter_MC
    df.loc[:, "weight_TT"] = df.weight_TT * df.XSWeight * df.METFilter_MC
    ...
    df.to_pickle("dataset_ww_weighted_both.pkl")
